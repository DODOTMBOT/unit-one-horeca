...
export async function GET(request: Request) {
  try {
    ...
    // Загружаем ВСЕ заведения системы
    const establishments = await prisma.establishment.findMany({
      include: {
        owner: { // Добавляем инфо о партнере
    ...
    const summary = establishments.map((est) => {
      ...
      const dateToCompare = todayStart.toDateString();

      if (type === "temperature") {
        const totalEquipCount = est.equipment.length;
        isFilledToday = est.tempLogs.some(
          (log) => new Date(log.date).toDateString() === dateToCompare
        );
        ...
        isFilledToday = est.healthLogs.some(
          (log) => new Date(log.date).toDateString() === dateToCompare && log.comment !== 'в'
        );
      ...
      return {
        ...
        city: est.city || "—",
        address: est.address,
        partner: est.owner ? `${est.owner.name} ${est.owner.surname || ''}` : "Система",
        isFilledToday,
    ...
    console.error("ADMIN HACCP API ERROR:", error);
