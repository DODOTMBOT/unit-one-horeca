...
export default function AdminHACCPHealthListPage({ searchParams: searchParamsPromise }: any) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          data.forEach((est: any) => {
            const cityName = est.city?.toUpperCase() || "НЕ УКАЗАН";
            initialExpanded[cityName] = true;
          });
  ...
  const groupedData = useMemo(() => {
    ...
    filtered.forEach(est => {
      const cityName = est.city?.toUpperCase() || "ДРУГИЕ ГОРОДА";
      if (!groups[cityName]) groups[cityName] = [];
      groups[cityName].push(est);
    });
    ...
    <div className="flex flex-col gap-6 pb-20 max-w-[1400px] mx-auto px-4 uppercase">
      ...
      <header className="flex flex-col md:flex-row items-center justify-between gap-6 py-4">
        <div className="flex items-center gap-5">
          {/* Кнопка теперь вызывает router.back() */}
          <button 
            onClick={() => router.back()} 
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight leading-none uppercase">Контроль здоровья</h1>
            <p className="text-sm text-gray-400 font-medium mt-1 tracking-wider">Группировка по городам</p>
          </div>
        </div>
                  ...
                  {isExpanded && group.items.map((est) => (
                    ...
                    >
                      <td className="px-10 py-5">
                        <Link href={`/partner/office/establishments/${est.id}/health`} className="flex flex-col group">
      ...
      <footer className="flex justify-between items-center px-4 mt-8 opacity-40">
        ...
        <div className="flex gap-4 items-center">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-[#10b981] uppercase">Health Monitor</span>
        </div>
      </footer>
    </div>
  );
