...
import { 
  ChevronLeft, ChevronRight, Home, LogOut, Search, Activity, ArrowRight
} from "lucide-react";
import Link from "next/link";
import { useSession } from "next-auth/react";

export default function PartnerHACCPHealthListPage({ searchParams: searchParamsPromise }: any) {
  ...
  useEffect(() => {
    async function load() {
      ...
      } catch (e) { console.error("Load failed"); } finally { setLoading(false); }
    }
    if (session) load();
        ...
        <header className="flex flex-col md:flex-row justify-between items-center gap-4 mb-10">
          ...
          <div className="px-10 py-3 bg-white border border-slate-100 rounded-full shadow-sm text-center">
            <h1 className="text-xs font-black uppercase tracking-[0.2em] text-slate-800 leading-none">
              Выбор заведения: Журнал здоровья
            </h1>
          </div>
              ...
              >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <span className={`text-[9px] font-black uppercase ${est.isFilledToday ? 'text-emerald-500' : 'text-rose-500'}`}>
                         {est.isFilledToday ? 'Заполнено сегодня' : 'Есть пропуски'}
                       </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                   <div className="hidden sm:flex flex-col items-end">
                      <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Пропуски за месяц</p>
                      <div className="flex gap-1">
                        {est.facilitySkipDays?.length > 0 ? (
                          est.facilitySkipDays.slice(0, 5).map((d: any) => (
                            <span key={d} className="w-5 h-5 flex items-center justify-center bg-rose-50 text-rose-500 rounded-md text-[8px] font-black">{d}</span>
                          ))
                        ) : (
